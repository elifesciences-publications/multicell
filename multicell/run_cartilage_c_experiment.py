...
system = RuleBasedModel(cell_size=0.02, mesh_resolution=160, mesh_type="unstructured", variance_division_time=0.01,
                        pushing_factor=0.1,mean_migration_time=1e20,gradient="onesided_linear",polarization_strength=1000)
...
for c in coeffs:
    ...
    for i in xrange(N):
        ...
        with open("cartilage/data/cartilage_c_{0}_{1}.pyb".format(c,i),"wb") as fh:
            fh.write(pickle.dumps(results))
        print "Competed one iteration"

#with open("cartilage/data/cartilage_c_{0}_scores.txt".format(c),"w") as fh:
#        fh.write(s)
